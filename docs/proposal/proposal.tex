...
\begin{document}
    ...
    We propose creating a generic data synchronization service comprised of a Sync Service, Identity Service, and Sync Client.
    ...
    The Sync Service will provide data synchronization, allowing us to reuse the same service to manage data synchronization for many different applications.
    ...
    Ideally to demonstrate the extensibility and versatility of our system, we will be able to write several (1-3) simple clients. % TODO: is this too optimistic/burdensome? should we say what they are?
    The focus of these clients will be demonstrating the system in different domains, so they won't have to be overcomplicated.
    ...
    \footnote{Interestingly I do not think this will necessitate using a relational database for the main Data Store, which only needs to retrieve tuples based on ownership. This is excellent for scalability.}
    ...
    Clients will be able to use regular relational operations to manipulate user data to fit the purposes of the UI.
    ...
    % It would be interesting to perform load testing on the server to demonstrate the scalability of the system and understand how viable it would be in a real world deployment, if we have time.
